...
function renderEducation(doc, education) {
  ...
  education.forEach((e, i) => {
    ...
    doc
      ...
      .fontSize(9)
      .fillColor("#000000")
      .text(`${e.degree}  |  ${e.dates}  |  `, { continued: true })
      .font("Helvetica")
      .fillColor("#555555")
      .text(e.school);
    doc.fillColor("#000000");
...
function renderCertifications(doc, certs) {
  ...
  certs.forEach((c, i) => {
    ...
    doc
      ...
      .fontSize(9)
      .fillColor("#000000")
      .text(`${c.name}  |  ${date}${issuer ? "  |  " : ""}`, { continued: !!issuer });
    if (issuer) {
      doc.font("Helvetica").fillColor("#555555").text(issuer);
    }
    doc.fillColor("#000000");
  });
}
...
function skillLine(doc, label, value) {
  if (!label) {
    doc.font("Helvetica").fontSize(8.5).fillColor("#000000").text(value);
    doc.moveDown(0.02);
    return;
  }
  doc
