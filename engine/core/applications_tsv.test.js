...
test("save + load round-trips v2 fields (salary_min, salary_max, cl_path)", () => {
  const file = tmp();
  const { apps: built } = apps.appendNew([], [fixtureJob()], { now: "2026-04-20T00:00:00Z" });
...
test("load auto-upgrades v1 files (12 cols) with empty v2 fields", () => {
  const file = tmp();
  ...
  // Re-saving promotes the file to v2.
