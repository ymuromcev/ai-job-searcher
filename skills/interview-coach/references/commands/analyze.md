...
12a. **Cross-Dimension Root Cause Check.** After scoring all units, scan for root causes that appear across 2+ answers (e.g., "conflict avoidance" affecting both Substance and Differentiation). Cross-reference with `coaching_state.md` → Calibration State → Cross-Dimension Root Causes (active). If a detected root cause already exists as an active entry, update its status and note whether affected dimensions are improving. If a new root cause is detected (same pattern in 2+ answers), create a new entry in the Calibration State table with a unified treatment recommendation. This ensures recurring root causes are tracked as systemic issues, not re-diagnosed per session. Every row carries a status (`active` / `improving` / `watching` / `closed` / `resolved`) so the debrief reminder hook can surface it before the next interview — a row with no recognized status is a defect `debrief_audit.js` flags.

12b. **Outcome-calibration pass (RFC 066 — the debrief loop).** This transcript's round has (or will have) an `## Outcome` block written at debrief. Reconcile it now against what actually happened:
    - **Channel.** Attribute the result to a channel: `LIVE` (what the interviewer heard on the call) or `PAPER` (what reached the next gate without the call — recruiter notes, resume, self-ratings said aloud). Do not prescribe live-delivery fixes for a loss at a gate that never heard the candidate. A failure can be LIVE-origin with a PAPER path — tag the channel that actually decided.
    - **Reconcile the prediction.** Compare the recorded `Predicted` to the result. An endorsement to the *next* gate is a filter, not a pass — score it `pending` until that gate resolves, never as an advance. A miss becomes a calibration lesson in Cross-Dimension Root Causes (step 12a), not a shrug. The scorecard delta feeds the score-trajectory read in step 16.
    - **Quarantine adverse conditions.** If the round ran under real adverse conditions (no sleep, cold English, va-bank), set `Baseline: quarantined` on the block so a condition-driven loss does not drag the skill trend — it is not proof of regression. Weigh the reschedule honestly (delay cost vs. whether the replacement slot was actually better) before excusing or blaming.

    The block's canonical shape and the exact loop rules live in `debrief.md` → "Outcome Block". `debrief_audit.js` enforces the closing fields; run `audit` (see `audit.md`) after writing the block.
