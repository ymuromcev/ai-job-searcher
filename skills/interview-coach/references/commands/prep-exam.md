...
3. **Grounding.** Find where the employer's domain overlaps something the candidate has actually run, and take **the situation, not just the numbers**, from there: what the decision was about, who owned it, what was at stake if it went wrong, what the team called things. For a card-portfolio round with a candidate who ran a credit-card funnel, the numbers are his funnel's — and so is the meaning: activation is the thing his relaunch moved, the stakeholder is the one who actually asked him for a verdict. Numbers alone give arithmetic to practise on; the situation is what makes the apex **already-known**, and there has to be something known to descend *from*. A dataset transplanted without its context is a word problem, and a word problem is bottom-up in disguise.
