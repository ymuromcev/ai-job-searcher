...
| `prep [company]` | Full prep brief — role-fit assessment (5 dimensions — identifies frameable vs. structural gaps), format guidance, culture read, interviewer intelligence (from LinkedIn URLs), predicted questions (weighted by real questions from past interviews when available), story mapping, and a day-of cheat sheet. Three flows: `prep screening` (sell to a recruiter), `prep manager` (sell to the hiring manager), `prep exam` (pass a technical round) |
| `prep exam [company]` | Exam-round prep for a closed discipline — statistics, A/B, SQL, risk metrics. Curriculum taken verbatim from the employer's own scope document and mapped to a textbook, drills on one running dataset from your experience, every answer graded out loud, cheat sheet with a ready spoken line per trap, and a T−4h freeze on new material |
| `audit` | Mechanical check of the open round files — what is still un-drilled, which debts are open, whether the cheat sheet validates. Run it before believing anyone who says you're ready, including me |
