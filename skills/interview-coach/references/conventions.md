...
Перед выдачей любого английского, который кандидат будет **читать вслух** (🗣️ блоки, произносимые ответы конспекта, `practice` / `mock`, EN-колонка STAR-таблицы в `Story Details`, `hype`-анкоры), калибруй словарь по **Reword mastered**. Три уровня, ни один не молчаливый:
...
- **C — регистр старшей должности.** Ответ звучит так, как говорит senior-менеджер в большой компании на хорошей позиции, а не просто беглый английский. Если у обычного слова есть деловой синоним **из mastered** — берём его (`got everyone to agree → got buy-in`, `the slow part → the bottleneck`, `who answers for it → accountability`, `our plan → the roadmap`). Разговорное в произносимом тексте (`stuff`, `things`, `a lot of`, `kind of`, `pretty`, `basically`, `get`) — заменить. Только слова из mastered: кандидат их знает, а повтор в ответах — это их активация. Регистр меняет **слово, не фразу**: предложения остаются короткими и простыми по правилу 1, одна деловая лексема на мысль, без нагромождения buzzwords (`leverage synergies` — нет). Регистр не усиливает сам факт — масштаб и авторство по-прежнему только из storybank (правила 13, 15).
...
3. **Применить три уровня.** Сравнение без учёта регистра, лемматизация свободная (`share` покрывает `sharing`, `shares`). **Никогда не трогать, не флагать и не класть в колоду:** доменный жаргон кандидата (funnel, conversion, monetization, retention, guardrail, proxy metric, north-star, cohort, MCP, CPA, affiliate, aggregator, …), имена собственные, числа, сами mastered-слова. **Сомневаешься — понижай до вотч-листа.** Никогда не подменять силой слово, которое может быть известно, и никогда молча не выбрасывать не-mastered слово.
...
5. **Форма вывода.** Под произносимыми ответами два компактных блока — 🔄 *Replaced* (A и C: что на что, почему; для C причина `register`) и 👀 *Check* (B: в ответе, нет в Reword mastered) — плюс строка про колоду. Пустой блок опускается.
...
- **Исключение — три вводных вопроса** (Q0 / Q1 / Q2). Они не предсказаны, а фиксированы скелетом и звучат в каждом раунде; их основание — сам этот факт, и писать его строкой не нужно ([`konspekt-skeleton.md`](konspekt-skeleton.md)).
...
**Жёсткий фильтр: вопрос задаётся только тому, кто может на него ответить.** Проверка одна — знает ли этот конкретный человек ответ по своей работе, а не «уместно ли это спросить». Вопрос не по адресу стоит кандидату дважды: он тратит один из четырёх слотов и получает «хороший вопрос, задай его на следующем интервью», то есть публично промахивается мимо того, с кем говорит.

- **Рекрутёру** — то, что лежит в его собственной работе: бриф нанимающего своими словами, новая роль или замена, кому подчиняется, формат и локация, состав и сроки процесса, сколько кандидатов уже в воронке, почему люди в компанию приходят и остаются. Продуктовые, доменные и технические вопросы — мимо: «что из трёх частей домена болит сильнее всего», «как устроена архитектура», «какая метрика у команды». Он их не знает, и знать не должен.
- **Нанимающему** — ровно то, что срезано у рекрутёра: что болит, как устроено, чем меряют, что считается успехом за 90 дней.

Хороший продуктовый вопрос не выбрасывается, а переезжает в конспект следующего раунда.
...
Перед выдачей проверить по всему файлу три вещи: (a) повествование каждого ответа — **причинно-следственная цепочка, которая держится** (причина → следствие, а не мешок утверждений подряд); (b) **ни один ответ не противоречит утверждению из другого места** конспекта или реальной истории кандидата — конспект, который продаёт одну рамку в Q7 и подрывает её в Q1, вручает интервьюеру противоречие; (c) **каждый вывод из справки исполнен в речи**.

**Про (c).** Строки в «Что из этого следует» (`📖 Кто интервьюер`) и стратегическая рамка из `📖 Что это за звонок` — это не наблюдения, а **инструкции к произносимому тексту**. У каждой должен быть адрес: конкретный ответ, докрутка, порядок историй или блок «если спросит», где она исполнена. Вывод без адреса — это либо лишняя строка в справке, либо неисполненная инструкция; и то и другое чинится до выдачи. Направление проверки одностороннее: справка обязывает речь, речь справку — нет.

**How to apply.** Как гейт выдачи: (1) прочитать каждый ответ и проверить, что биты складываются в «потому что», а не в список; (2) вычесать любое утверждение, которое опровергается *другим* ответом или известной историей кандидата — «впервые», «не делал», «для меня новое» при том, что другой ответ говорит обратное, классический признак; (3) выписать столбиком все выводы из «Что из этого следует» и стратегическую рамку из «Что это за звонок», и против каждого назвать место в 🗣️, где он исполнен. Не назвалось — либо исполнить в речи, либо убрать вывод из справки. Чинить до выдачи. Часть DoD любого конспекта.

**Почему (c) дописано отдельно.** Проверка (b) ищет противоречия *между ответами* и расхождение «справка обещает — речь не делает» пропускает: формально ничему не противоречит, а инструкция не исполнена. Дефект, на котором правило дописано (Mitratech Catalyst, 2026-09-23): в справке про интервьюера стояло «четыре темы из его письма — это заранее одобренная повестка, начинать надо с них», а открывающий ответ нёс хуки, выведенные из вакансии, и две из четырёх названных тем стояли в хвосте списка историй. Поймал кандидат, читая готовый конспект.
...
**«Дочитываешь сразу» не пересказывает историю.** История транслюдируется целиком и звучит вслух — всё, что в ней есть, кандидат уже произнёс. Дочитка существует ровно для одного: перевести историю на язык этой вакансии. В ней только то, чего в истории нет и быть не может — где в их продукте то же место, тот же клиент, тот же стык. Цифры, «я вёл спецификацию», формулировки вывода — всё это уже прозвучало, и повтор через десять секунд звучит как заученный текст. Если моста нет и добавить нечего, дочитки нет вообще: это необязательный блок.

**How to apply.** Как гейт выдачи: выписать по одной фразе смысл каждого абзаца справочных разделов и сравнить список на дубли. Для дочиток — читать каждую подряд с текстом истории и вычёркивать любую фразу, которая уже прозвучала выше. Совпало — оставить в разделе-владельце, из второго вычеркнуть; частично совпало — оставить в одном месте полностью, во втором не оставлять ничего. Часть DoD любого конспекта, рядом с `14`.
...
- **Название истории берётся из storybank дословно.** Заголовок блока в конспекте — это порядковый номер (навигация) плюс название из storybank как есть. Служебный префикс `S0nn — ` в конспект не переносится: идентификатор нужен внутри storybank, а кандидату в заголовке он только мешает. Переименовывать, сокращать, обезличивать и пересказывать результатом запрещено. Причины: (1) кандидат ищет историю по названию, и в конспекте оно должно совпадать с тем, что он видит в storybank; (2) каждое переименование теряет что-то существенное — глагол («построил и вёл» → «пять команд»), предмет («ФЗ-218, ИНН для скоринга» → «регуляторный срок»), инструмент («через Claude Code» → пусто), сегмент («B2B-маркетплейс» → «маркетплейс»); (3) без этого одна история носит столько имён, сколько было раундов. Название кажется мелочью, поэтому переписывается на автомате — и именно поэтому запрет нужен явный. Название неудачное — правится в storybank по запросу кандидата (`15a`), а не локально в конспекте.
...
- **Storybank правится только по прямому запросу кандидата.** Это его файл и его слова. Claude не переписывает в нём ни строки по собственной инициативе — ни «заодно», ни «раз уж нашли», ни в порядке применения только что согласованного правила. Нашёл дефект — называет строку кандидату вместе с предложенной заменой и ждёт прямого «да». Единственное исключение — история ещё не в say-aloud форме и помечена соответствующим флагом: это и есть санкционированный polish-проход, описанный выше.
...
### Владение звучит метрикой, а не поставкой · 15a

**Флоу:** screening · manager. Живёт в тексте истории.

Реплика вида «я вёл спецификацию и запустил интеграцию целиком» описывает поставку и читается как проджект-менеджер: сделал, отдал, дальше не моё. Продакта отличает не то, что он запустил, а то, за какое **число** он потом отвечал.

Поэтому в каждой истории строка про владение называет и то, и другое, и в одной реплике: что было на нём по запуску **и** какая бизнес-метрика была его после запуска. Формулировка кандидата, эталон: «Техническую спецификацию вёл я, и запуск интеграции был на мне. После запуска я отвечал за конверсию из отказа в кредитке в переход к партнёру». Двух частей достаточно — отрицания («отвечал не за поставку, а за…») не нужно, оно только привлекает внимание к тому, чего не было.

Обратная ошибка — раздувание — разбирается в `15`: зона называется точно, не шире реальной. Два требования работают вместе: **не уже поставки и не шире реальной ответственности.**

**How to apply.** Дефект такого рода живёт в самой истории, и правка в обвязке конспекта его не чинит: обвязку следующий конспект перепишет, а история транслюдируется дальше как есть. Но чинить её молча нельзя — storybank правится только по прямому запросу кандидата (`28`). Найденную строку Claude показывает вместе с предложенной заменой и ждёт «да»; до ответа в конспекте ничего не подпирается обходным путём.

↩ История: `decisions-log.md` § 15a.

### Сравнение не уменьшает собственную цифру · 15b

**Флоу:** screening · manager. Живёт в тексте истории — правка по запросу кандидата (`28`).

История сравнивала свой результат с соседней фичей: «моя дала 6% и 4000 активаций за 3 месяца, а соседняя — 10% и 7000 за год». В голове рассказчика это про скорость. В голове слушателя остаётся «10 и 7000 больше, чем 6 и 4000», и запоминается чужая цифра, а не своя.

Сравнение в истории допустимо только тогда, когда собственное число в нём **не меньше**. Если своё меньше, а гордость — за срок или за цену, то сравнение убирается, а срок остаётся в той же фразе, что и результат: «6% роста, 4000 активаций в месяц, 3 месяца от идеи до запуска». Скорость слышна и без чужого числа рядом.

Отдельно это ломается под нагрузкой на английском: четыре числа в одной реплике не удерживаются ни рассказчиком, ни слушателем.

↩ История: `decisions-log.md` § 15b.
