...
| `prep [company]` | Build role-specific prep brief — always opens with the mandatory 8-step company + role research screen. **Pick the flow first**: `prep screening` (recruiter), `prep manager` (hiring manager), `prep exam` (technical / quantitative round). The three are mutually exclusive; a mixed call gets two files, not a hybrid | 8-step research front-matter (business model, primary-source JD + anti-requirements, people, fit map, risks/landmines, verified/unverified reputation, culture vocab, comp), then predicted Qs, story mapping, concerns, your questions, day-of cheat sheet |
| `prep exam [company]` | Prep for a round that is an exam in an exact subject (statistics, A/B testing, SQL, finance) — teach from a pinned canonical source, grounded in the candidate's own data | Round file with a curriculum table (topic → source section → status `не начат / объяснён / отработан вслух`), one running dataset, graded numeric drills, a debts ledger, and a separate `..._ШПАРГАЛКА.md` cheat sheet |
| `audit` | Run the mechanical detectors — round files before any readiness claim, and the debrief loop after an interview | **Round (13):** missing source, unstarted topics, open debts, deleted debt ids, formula without derivation, missing spoken line, topic absent from the cheat sheet, forbidden section for this `kind`, write inside the T−4h freeze, topic no case reaches, unroll node backed by nothing, apex naming the apparatus, cheat-sheet entry with no situational trigger. **Debrief loop (4):** outcome without LIVE/PAPER channel, outcome unreconciled against its prediction, adverse-condition loss not quarantined, root cause with no carry-forward status |
...
| `debrief` | Post-interview rapid capture (same day) | Questions recalled, interviewer signals, stories used, coaching state updates, and an `## Outcome` block (prediction + channel + conditions) that the debrief loop closes when the gate resolves |
...
- `Interview Format` (with open-format coaching boundaries — for an exam round use `prep exam`, where teaching and grading the subject are the coach's job, not out of scope)
...
├── templates/
    ├── profile-readme.md               # Per-candidate entry-point doc, regenerated by the skill
    ├── round-exam.md                   # Round file for an exam round (curriculum table, debts, readiness DoD)
    ├── round-screening.md              # Round file for a recruiter screen
    └── round-manager.md                # Round file for a hiring-manager round
└── scripts/
    ├── round_audit.js                  # The 9 round-file detectors + `audit` CLI
    └── hooks/
        ├── prep_gate.js                # UserPromptSubmit — injects the exam entry protocol when a prep prompt is detected
        └── prep_freeze_guard.js        # PreToolUse — blocks writes inside the T−4h freeze and formulas with no derivation pointer
```
