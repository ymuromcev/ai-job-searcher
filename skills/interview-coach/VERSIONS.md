...
- **2026-08-05** — детектор 8 в `scripts/round_audit.js` (продающая секция внутри `kind: exam`) научился ловить оба формата конспекта: исторические буквенные секции и story-centric скелет («## 📖 Справка по компании», «## 🗣️ Вводные вопросы»). После смены формата 2026-08-04 он проверял только названия старой схемы и на новом конспекте молчал. Key files: `scripts/round_audit.js` (+ test), `references/commands/prep.md` (§ Mutual exclusion).
- **2026-08-04 (рефакторинг спеки)** — `conventions.md` перестала быть журналом инцидентов. 34 правила сгруппированы по артефакту (конспект · истории и storybank · правда и атрибуция · разборы и петли · экзамены), исходные номера сохранены алиасами и проиндексированы, у каждого правила помечен флоу (`screening` / `manager` / `exam`). Вся история — даты, инциденты, цитаты кандидата — вынесена в новый `references/decisions-log.md`. Формат конспекта вынесен в новый `references/konspekt-skeleton.md` и описан там единственный раз: раньше он жил в правиле 28 и в 28a и успел разъехаться сам с собой. Мёртвая `Output Schema` из `prep.md` (схема документа, который не выпускается с 2026-08-03) удалена целиком — остался исследовательский протокол, карта «шаг ресёрча → раздел конспекта» и режимы triage/focused/full. 1444 + 836 строк → 783 + 601 + 359 + 143. Key files: `references/conventions.md`, `references/decisions-log.md` (new), `references/konspekt-skeleton.md` (new), `references/commands/prep.md`, `SKILL.test.js`, `templates/round-{screening,manager}.md`, `SKILL.md`, `README.md`.
