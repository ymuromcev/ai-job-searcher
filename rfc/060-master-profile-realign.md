...
| 2 | Fit по достижениям | Перепаять скоринг фита: оценивать вакансию против РЕАЛЬНЫХ достижений (`fit_profile.md` digest из storybank), а не рукотворного доменного rubric. `user_resume_key_points.md` понижен до fallback. Хард-фильтры/блокеры остаются | storybank.js, fit_digest.js, fit_profile.js, build_fit_profile.js, prepare.js, validate.js, profile_loader.js, SKILL.md, pipeline-fit-evaluator | BL-192 | ✅ done 2026-06-15 (PR #63 foundation + fit-rewire) |
...
- **master_profile.md:** ~~achievement-секция тоже тянется из банка~~
  **ДРОПНУТО 2026-06-15 (решение Джареда).** Аудит потребителей показал: оба,
  кому нужны достижения, уже читают банк — фит через `fit_profile.md` digest,
  тейлоринг (`resume-tailor-mirror`) получает `storybank_path` напрямую вместе с
  `master_profile_path`. Доп. achievement-секция в мастере = третья копия тех же
  фактов, что противоречит принципу «один источник». Ни один потребитель не
  читает мастер в обход банка → секция не строится. `resume_versions.json`
  остаётся источником ПОДАЧИ: 16 архетипов, career_narrative, contact, education.
  Если появится потребитель, читающий мастер без банка — добавим тогда (YAGNI).
...
- ~~`build_master_profile.js`: achievement-секция тянется из банка.~~
  **ДРОПНУТО 2026-06-15** — см. выше (дублирование, оба потребителя уже читают банк).
...
### Модель архетипов (Stage 3 / BL-191, зафиксирована 2026-06-15)

Архетип ≠ master-идентичность. Архетип = **дешёвый честный тейлоринг под НИЗКИЙ
фит**: версия CV, чтобы продать кандидата конкретному кластеру по максимуму без
вранья. Логика **обратная** master'у: архетип наклоняется в то, что хочет кластер,
а не в anchor-голову. Anchor («AI=HOW, ecom-голова») рулит **только мастером**.

- WHAT архетипа МОЖЕТ отличаться от мастера (AI/fintech/analytics в заголовке) —
  **если пруф реальный** (AI-архетип Jared'а ведёт AI: AI Job Searcher, MCP-серверы,
  AI-прескоринг — настоящие).
- Создаём/держим архетип только при честном крючке; нет пруфа — не выдумываем,
  дропаем. Тонкий честный трансфер (regulated funnels → healthcare и т.п.) —
  оппортунистичный мост, решение держать его **за кандидатом**.
- Бакеты — из реальных достижений (банк), не сочиняем под кластер.
- Зеркалим титулы кластера (CalCareer gov-роли = «IT Delivery Manager» — оставляем).
- Без инфляции (Senior остаётся Senior, не Staff).

**Где правило живёт (в туле, не в памяти):** канон — `docs/reference/archetypes.md`;
применяется при онбординге (`skills/onboard-profile` Block 3); указатели — в
`scripts/stage18/generators/resume_versions.js` (механический маппер, не решает
позиционирование) и `skills/job-pipeline` Step 7 (только выбор ключа). Причина
фиксации в коде/доке: автоматическое поведение не должно зависеть от памяти/чата —
в этой сессии master-правило по ошибке применили к архетипам, ровно эту путаницу
док и снимает.

**Решения по составу (Jared, 2026-06-15):** все 16 архетипов сохраняются (THIN —
HealthTech/LogisticsOps/Crypto/TrustSafety — оставлены как честные мосты);
SpendCards Staff→Senior; Govtech оставляет «IT Delivery Manager» (требование
CalCareer). Полный целевой набор (16 + возможные новые непокрытые кластеры) —
готовит Claude, согласование до правки `versions{}`.
...
- **Стадия 2 — ✅ done 2026-06-15.** Генератор digest из банка (freshness-хэш,
  pre-prepare) + перепайка `pipeline-fit-evaluator` + SKILL Step 4 на
  achievement-overlap + coverage-тест в `validate`. PR #63 (foundation) + PR #65
  (fit-rewire). Пункт «achievement-секция в master_profile.md» **дропнут**
  (дублирование — оба потребителя уже читают банк). BL-192 закрыт.
- **Дальше — стадия 3 (BL-191):** прогон на пуле под новый фит → реальные кластеры
  Strong/Medium → пересмотр 16 архетипов под них + hard-фильтры `filter_rules.json`
  под реальный домен (bridge-треки BL-37 НЕ трогаем).
- ~~**Открытый вопрос на потом:** master profile achievement-секция~~ **RESOLVED
  2026-06-15:** дропнута. Не генерим из банка и не держим кластеры с coverage-guard'ом
  в мастере. Причина — оба потребителя достижений (фит, тейлоринг) уже читают банк
  напрямую; секция в мастере была бы третьей копией фактов против принципа единого
  источника. Стадия 2 закрыта на fit-rewire (PR #65); BL-192 done.
