...
> До 2026-05-04 в этой таблице была роль `PUSH_SKIP_STATUSES = {Archived}` для `sync` push-фазы. Push удалён (commit `4f85ed2`); страницы теперь создаёт только `prepare`, и его fresh-row gate (см. CC-1.a) уже исключает `Archived` — отдельная роль не нужна.
...
- `engine/commands/sync.js:1-16` — header comment: sync pull-only, push removed 2026-05-04; нет PUSH_SKIP_STATUSES.
...
**Current implementation**: `engine/commands/prepare.js` — фильтр fresh-rows: `status === "To Apply" && !notion_page_id` (PM-9.1). Историческая параллельная защита `sync.planPush` с gate'ом по `cl_key OR resume_ver` (commit `d2f7d92`) удалена 2026-05-04 вместе с push-фазой.

**Gap**: Двойной смысл `"To Apply"` создаёт риск race'ов и confusion'а у пользователя при ручной работе в Notion. Окончательное решение — RFC 012 (нормализованная модель с `prepared_at` timestamp). Сейчас защищено единственным guard'ом — fresh-row фильтр в `prepare` (sync push удалён, дублирование больше не нужно).
...
- `engine/core/applications_tsv.js:appendNew()` — dedup по `key`, default `"To Apply"`.
...
**Gap**:
- G-24: Pull не обрабатывает **deletion** (page deleted в Notion → local TSV не trim'ится). Эквивалентно прототипу (тот тоже не обрабатывал). Не баг, но требует documenting в RFC.
...
| G-23 | P-7 | Low | Canonical archetype gate ловит ошибку только в момент создания Notion-страницы (теперь — в prepare commit-фазе вместо удалённой sync push). Связан с G-18. Fix policy: ранний enforce'мент в SKILL Step 7. |
| G-24 | Sy-1 | Low | Pull не обрабатывает Notion deletion. Эквивалентно prototype. Не gap, но spec'ить в RFC. |
...
**Итого**: 12 cross-cutting + 9 scan + 9 prepare + **2 sync** (pull + property mapping; 4 контракта удалены вместе с push-фазой 2026-05-04) + 9 check + 5 validate = **46 контрактов**, **33 gap'а** в Open issues — из них **3 закрыты архитектурным rollback'ом sync push** (G-21, G-22, G-25).

**Архитектурные divergences vs prototype** (для приоритезации в Phase 3):
- **High**: G-7 geo enforcement, G-17 CL generation paradigm.
- **Medium**: G-1 status double-meaning, G-3 title_requirelist fragmentation, G-4 fuzzy dedup unused, G-10/G-11 SKILL interactive prompts, G-15 unknown-tier silent pass-through, G-18 archetype selection, G-33 location_blocklist retro. (~~G-21 double-write~~, ~~G-22 split property API~~ закрыты 2026-05-04 — sync вернулся к prototype's pull-only архитектуре.)
