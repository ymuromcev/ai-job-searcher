...
- Slash-split — engine улучшение **без явного источника**. Помечается gap'ом — нужна цитата пользователя или RFC, которого нет. Поведение полезное и ничего не ломает; до триажа оставляем.
...
| G-2 | CC-3.1 | Low | Slash-title split — engine improvement без явного источника в RFC/incident. Поведение полезное, оставляем; добавить в RFC 012 ретро-обоснование. |
...
| G-5 | CC-6 | Low | `applications.tsv` без колонки `location` — окончательный fix в RFC 012. |
...
| G-26 | C-4 | Low | LinkedIn branch создаёт `"To Apply"` rows c `url=""` → каждая такая row 100% умирает на prepare URL-check (G-13). Cross-cut, не fixable без либо (а) выбора source-skip в prepare, либо (б) URL-resolution из LinkedIn body. |
