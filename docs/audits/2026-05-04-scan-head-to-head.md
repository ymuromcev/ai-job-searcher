---
title: "Scan command head-to-head"
status: archived
dated: 2026-05-04
tags: [audit, scan]
---
...
**Triggers** post-compact: "test scan head-to-head" / "continue the scan test" — already done; re-run only if a regression suspect surfaces.
**Goal**: verify that the engine `scan` produces the same fresh-jobs set as the prototype on the same date, with location parity (after G-5) and no LinkedIn-empty-URL rows (after G-26).

---

## Result snapshot — 2026-05-04 (PM-Pete profile)

**Engine**: 19 476 pool rows, 37 fresh today, filter rejected all 37 (35 title_requirelist for non-PM titles, 1 company_blocklist, 1 title_blocklist) — correct behavior.
...
- 693/756 (≥92%) of the prototype's PM-relevant universe is present in the engine pool.
...
- All 59 "Senior+/Mid/Analyst PM → READY FOR NOTION" prototype rows from this scan are 100 % present in the engine pool (URL or `source:jobId` match).

**Real source-coverage gaps** (not regressions — known absences):
1. **CalCareers** — adapter does not exist in the engine. ~58 jobs/scan miss.
2. **USAJobs** — adapter exists but inactive (requires `<PROFILE_ID>_USAJOBS_API_KEY` + email registration on usajobs.gov). ~22 jobs/scan miss.
...
1. **Confirm clean state** — `git status` should not show pending edits in `engine/`, `scripts/`, `docs/`. Last green test run = 805/805 on the commit after G-5.
2. **Confirm schema v3** in both profiles:
   ```
   head -1 profiles/<id1>/applications.tsv | grep -c location  # → 1
   head -1 profiles/<id2>/applications.tsv | grep -c location  # → 1
   ```
3. **Backups exist**: `profiles/<id1>/applications.tsv.pre-stage-g5` and `profiles/<id2>/applications.tsv.pre-stage-g5` (created during G-5 backfill 2026-05-03).
...
```bash
cd "ai-job-searcher"
node engine/cli.js scan --profile <id1> --dry-run > /tmp/scan_jared_engine.log 2>&1
node engine/cli.js scan --profile <id2> --dry-run > /tmp/scan_lilia_engine.log 2>&1
```
...
Save the tail of each log into the plan as artifacts.

## Phase 2 — prototype scan (in parallel)

```bash
cd "../Job Search"  # PM-Pete prototype
node find_jobs.js --dry-run 2>&1 | tee /tmp/scan_jared_proto.log

cd "../Lilly's Job Search"  # Healthcare-Hannah prototype
...
If the prototype lacks a dry-run gate, skip Phase 2 and rely on the most recent prototype TSV snapshot as reference.
...
Write the diff result to `/tmp/scan_diff_<profile>.md`:
```
...
After the dry-run engine scan, sample 10 fresh jobs and verify each carries a non-empty `location` from the adapter (look at the `(dry-run) would append…` log + cross-check master pool format).
...
```
Should return zero. LinkedIn ingestion is disabled — no fresh `source=linkedin` rows should appear from scan (only from the check.js path, which now also short-circuits).

## Phase 6 — decision

Based on Phases 3-5:
- **All clean** → proceed to test `prepare` next, then `sync --apply` against a fresh batch.
- **P \ E non-trivial** → prioritize fixing the adapter regression before further testing.
