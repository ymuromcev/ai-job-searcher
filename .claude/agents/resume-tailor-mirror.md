...
- **Personal Projects**: render as ONE section with a single prose paragraph (no per-project bullets). Lead with the most JD-relevant project bolded, then bridge to the others with ` ` separators. **URL rule**: when the block combines more than one project, set the project `url` to the candidate's GitHub/org **profile root** (e.g. `github.com/ymuromcev`), never a single-repo deep-link (`github.com/ymuromcev/ai-job-searcher`) — the header must not appear to link to just one of several listed projects. Use a single-repo URL only when the section genuinely describes exactly one project.
